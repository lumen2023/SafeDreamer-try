...
class WandBOutput:
  ...
  def __init__(self, logdir, config, pattern=r'.*'):
    ...
    wandb.init(
        ...
        name=f"imag_horizon:15-horizon: 5, num_samples: 1000, mixture_coef: 0.1, mixture_coef_pi: 0.1, mixture_coef_gaus: 0.9, mixture_coef_idm: 0.0, num_elites: 50_{datetime.now().strftime('%m%d-%H%M')}",  # 只保留日期+小时
