...
class WandBOutput:
  ...
  def __init__(self, logdir, config, pattern=r'.*'):
    ...
    wandb.init(
        ...
        name=f"MoE-5-4-1-imag_horizon:15-horizon: 10, num_samples: 500, mixture_coef: 0.1, mixture_coef_pi: 0.5, mixture_coef_gaus: 0.4, mixture_coef_idm: 0.1, num_elites: 50_{datetime.now().strftime('%m%d-%H%M')}",  # 只保留日期+小时
